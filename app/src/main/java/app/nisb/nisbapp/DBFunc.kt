...
class DBFunc {
    ...
    fun makeTables(c: Context) {
        try {
            ...
            //user table
            db.execSQL("create table if not exists user(email text,ieeeno int,data text)")
            //notification table
    ...
    fun doUserLogin(c: Context, email: String, ieeeno: String, data: String): Int {
        try {
            val db = c.openOrCreateDatabase(database, Context.MODE_PRIVATE, null)
            makeTables(c)
            db.execSQL("insert into user values('$email',$ieeeno,'$data')")
            db.close()
            addNotification(c, welcomeNotif!!.toString())
            FirebaseMessaging.getInstance().subscribeToTopic("main")
            return 1
    ...
    fun addNotification(c: Context, data: String) {
        try {
            System.out.println("insert into notifs values('$data')")
            val db = c.openOrCreateDatabase(database, Context.MODE_PRIVATE, null)
